...
type MOQTMessage struct {
	MType   uint64
	Payload []byte
}

func (moqtm *MOQTMessage) Read(r io.Reader) error {
	...
	log.Printf("[GOT MTYPE - %0x][%s]", mtype, GetMoqMessageString(mtype))

	moqtm.Payload, err = io.ReadAll(r)
